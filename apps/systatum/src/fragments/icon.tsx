...
export const Icon: React.FC<IconProps> = ({
  ...
  const iconSizeClasses =
    typeof size === "string"
      ...
      : iconSizeClass[
          Object.keys(iconSizeClass)[size] as keyof typeof iconSizeClass
        ];
  ...
  if (style == "circle") {
    const circleClass =
      iconColorClass[iconColor as keyof typeof iconColorClass]?.circle ?? "";

    return (
      <div
        {...(tinaField ? { "data-tina-field": tinaField } : {})}
        className={`relative z-10 inline-flex items-center justify-center shrink-0 ${iconSizeClasses} rounded-full ${circleClass} ${className}`}
      >
